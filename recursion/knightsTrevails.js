...
class knightTree {
	...
	search = (position = this.root, queue = []) => {
		...
		const newPositions = possibleMoves.map(
			(x) => new knightPosition(x, position),
		);
		for (const i of newPositions) {
			const positionToken = i.coord.toString();
			if (this.end.toString() == positionToken) {
				return i;
			}
	...
	discoverMoves = (origin, array) => {
		...
		const filteredMoves = initialMoves.filter(
			(coord) =>
				validCoords(coord) && !this.discovered.has(coord.toString()),
		);
