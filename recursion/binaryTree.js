class Node {
	...
	get children() {
		const output = [];
		if (this.left) {
			output.push(this.left);
		}
		if (this.right) {
			output.push(this.right);
		}
		return output;
	}
}
...
class Tree {
	...
	inOrder(node = this.root, funct = false) {
		...
		let output = [];
		if (node.left) {
			output = output.concat(this.inOrder(node.left, funct));
		}
		if (funct) {
			funct(node);
		}
		output.push(node);
		if (node.right) {
			output = output.concat(this.inOrder(node.right, funct));
		}
		return output;
	}
	...
	preOrder(node = this.root, funct = false) {
		...
		output.push(node);
		if (node.left) {
			output = output.concat(this.preOrder(node.left, funct));
		}
		if (node.right) {
			output = output.concat(this.preOrder(node.right, funct));
		}
		return output;
	}
	...
	postOrder(node = this.root, funct = false) {
		...
		let output = [];
		if (node.left) {
			output = output.concat(this.postOrder(node.left, funct));
		}
		if (node.right) {
			output = output.concat(this.postOrder(node.right, funct));
		}
		if (funct) {
			funct(node);
		}
