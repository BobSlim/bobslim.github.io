...
export const shipCoords = (length, location, direction) =>
	!location | !direction
		...
		: [...Array(length).keys()].map((x) =>
				add(location, scale(direction, x)),
			);

export const getKeys = (array) => array.map(([key, value]) => key);
const getValues = (array) => array.map(([key, value]) => value);
const excludeValue = (array, excludedValue) =>
	array.filter(([key, value]) => value != excludedValue);
export const isOccupied = (map, coord) =>
	new Set(getKeys(map).map((x) => x.toString())).has(coord.toString());
...
export const Fleet = (boardSize = 10) => {
	...
	const place = (ship, location, direction) => {
		...
		if (invalidPlace(locations)) {
			return false;
		}
		shipCoordinates = shipCoordinates.concat(
			locations.map((x) => [x, ship]),
		);
		return shipCoordinates;
	};

	const invalidPlace = (locations) =>
		locations.some(
			(coord) => fleetIsOccupied(coord) | !isPointValid(coord, boardSize),
		);
	...
	const ships = () => [...new Set(getValues(shipCoordinates))];

	const isAllSunk = () => ships().every((x) => x.isSunk());
