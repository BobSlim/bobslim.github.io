...
const gameboard = (() => {
	...
	const turnHistory = [];
	const gameCells = [...document.getElementsByClassName("gamecell")].map(
		(element) => gamecellFactory(element),
	);
	gameCells.forEach((element) => {
		element.gamecellHTML.addEventListener("click", () =>
			playerClaim(element),
		);
	});
	...
	const checkWin = (grid, cell) => {
		...
		const winning = {
			...
			generatedLines: [],
			isWin() {
				return Object.values(this.wincons).includes(true);
			},
		};
